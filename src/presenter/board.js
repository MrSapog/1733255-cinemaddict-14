...
export default class FilmBoard {
  ...
  _renderFilms(from, to, list) {
    this._films
      .slice(from, to)
      .forEach((film) => this._renderFilm(list, film));
  }
  _renderAllFilmsList() {
    this._renderFilms(0, Math.min(this._films.length, RENDER_FILMS_COUNT), this._allFilmsListContainer);
    if (this._films.length > RENDER_FILMS_COUNT) {
  ...
  //   this._renderFilms(0, Math.min(this._films.length, RENDER_EXTRA_FILMS_COUNT), topRatedFilmsList);
  // }
